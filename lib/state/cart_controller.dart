...
class CartController extends ChangeNotifier {
  ...
  // ======================================================
  // ADICIONAR COM SUPORTE A VARIAÇÃO E PREÇO
  // ======================================================
  void add(
    Product product, {
    int quantity = 1,
    ...
    double? priceOverride, // ✅ NOVO: Recebe o preço da variação
  }) {
    ...
    if (index >= 0) {
      _items[index] = _items[index].copyWith(
        ...
        // Atualiza preço se mudou (opcional, mas seguro)
      ...
      _items.add(
        CartItem(
          ...
          priceOverride: priceOverride, // ✅ Salva o preço correto
        ),
      );
    }
  ...
  void increment(Product product, {int? variationId}) {
    final index = _items.indexWhere(
      (i) => i.product.id == product.id && i.variationId == variationId,
    );
  ...
  void decrement(Product product, {int? variationId}) {
    final index = _items.indexWhere(
      (i) => i.product.id == product.id && i.variationId == variationId,
    );
  ...
  void remove(Product product, {int? variationId}) {
    _items.removeWhere(
      (i) => i.product.id == product.id && i.variationId == variationId,
    );
    notifyListeners();
